...
import { IconArrow, IconSearch } from '../components/ui/Icons.jsx';
import { MalzemeGlyph } from '../components/ui/MalzemeGlyph.jsx';
import { useStore } from '../store/store.jsx';
import { getMalzemeAd } from '../../domain/entities/material.js';

/**
 * Bottom-sheet modal for material selection that slides up from the bottom.
 * Uses z-index 200 — sits above the form drawer (100).
 * @param {object} props
 * @param {() => void} props.onClose - Called when the backdrop is tapped or a pick is made
 * @param {(id: string) => void} props.onPick - Called with the selected material's ID
 */
export function SeciciSheet({ onClose, onPick }) {
    ...
    >
      ...
      >
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '4px 20px 24px' }}>
          ...
          {filtered.length === 0 && (
            <div style={{ padding: 30, textAlign: 'center', color: TOKENS.inkMuted, fontFamily: TOKENS.font, fontSize: 14 }}>Sonuç yok</div>
          )}
        </div>
      </div>
    </div>
