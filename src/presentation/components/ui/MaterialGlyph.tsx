...
interface MaterialGlyphProps {
  material: Material | null | undefined | { kind?: string; grade?: string; diameter?: string; name?: string; category?: string };
  size?: number;
}

export function MaterialGlyph({ material, size = 36 }: MaterialGlyphProps) {
  const s = size;
  ...
  if (material?.kind) {
    const { kind, grade } = material;
    const fill = grade === 'Siyah' ? '#1F1A14' : grade === 'Galvaniz' ? '#A9B0B8' : '#D0D5DA';
    if (kind === 'Boru')
  ...
  } else if (material?.category === 'Elektrod') {
    const isBaz = (material as { name?: string }).name?.includes('Bazik');
  ...
  } else if (material?.category === 'Boya') {
    const c = (material as { name?: string }).name?.includes('Kırmızı') ? '#C0392B' : '#7B7D7E';
  ...
  } else if (material?.category === 'Bağlantı') {
    if ((material as { name?: string }).name?.includes('Cıvata'))
