...
import { TOKENS, stokDurum, btnGhostStyle, btnDangerStyle } from '../components/ui/tokens.jsx';
 ...
 * @param {object} props
 * @param {{ label: string, onClick?: () => void }[]} props.items - Breadcrumb items
 */
 ...
 * @param {object} props
 * @param {string} props.k - Label
 * @param {React.ReactNode} props.v - Value
 * @param {boolean} [props.last] - When true, the bottom border is omitted
 */
...
/**
 * Card section heading — grey background, uppercase.
 * @param {object} props
...
/**
 * Large stock level bar — marks the minimum threshold at the midpoint.
 * @param {object} props
 * @param {number} props.stok - Current stock
 * @param {number} props.min - Minimum stock threshold
 * @param {string} props.renk - Fill colour
 */
...
/**
 * Movement history timeline — lists delivery and usage records chronologically.
 * @param {object} props
 * @param {object[]} props.hareketler - Array of movement records
 * @param {object} props.m - Related material object (used for the unit label)
 */
...
/**
 * Material detail page — shows stock status, properties, and movement history.
 * @param {object} props
 * @param {string} props.id - ID of the material to display
 * @param {(kind: string, id?: string) => void} props.open - Form modal open handler
 * @param {() => void} props.goBack - Navigate back to the stock list
 */
export function DetayPage({ id, open, goBack }) {
  const { getMalzeme, hareketlerFor, removeMalzeme } = useStore();
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '320px 1fr', gap: 20, marginTop: 16, alignItems: 'start' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ background: TOKENS.paper, border: `1px solid ${TOKENS.line}`, borderRadius: 12, overflow: 'hidden' }}>
            ...
            {m.tur ? (
              <><Row k="Çap" v={m.cap} /><Row k="Tür" v={m.tur} /><Row k="Cins" v={m.cins} /><Row k="Birim" v={m.birim} /><Row k="Minimum stok" v={`${m.minimum} ${m.birim}`} last /></>
            ) : (
              <><Row k="Kategori" v={m.kategori} /><Row k="Birim" v={m.birim} /><Row k="Minimum stok" v={`${m.minimum} ${m.birim}`} last /></>
            )}
          </div>
