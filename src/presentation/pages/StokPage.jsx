...
import { TOKENS, stokDurum, btnPrimaryStyle } from '../components/ui/tokens.jsx';
...
/**
 * Sortable table header cell with an active-sort indicator.
 * @param {object} props
 * @param {React.ReactNode} props.children - Heading text
 * @param {string} [props.sortKey] - Currently active sort key
 * @param {'asc'|'desc'} [props.dir] - Sort direction
 * @param {string} [props.k] - This column's sort key
 * @param {() => void} [props.onClick] - Column click handler
 * @param {number} [props.w] - Width in pixels
 ...
 * @param {object} props
 * @param {string} props.k - Heading label
 * @param {string|number} props.v - Primary value
 * @param {string} props.sub - Sub-label description
 * @param {boolean} [props.warn] - When true, renders the value in red
 */
...
/**
 * Mini horizontal bar showing the stock level relative to the minimum threshold.
 * @param {object} props
 * @param {object} props.item - Material object (requires stok and minimum fields)
 */
...
/**
 * Labelled chip filter group — single-select filter control.
 * @param {object} props
 * @param {string} props.label - Left-hand group label
 * @param {string} props.value - Selected value
 * @param {(value: string) => void} props.onChange - Selection change handler
 * @param {string[]} props.options - Option list
 */
function FilterGroup({ label, value, onChange, options }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
      ...
      {options.map((o) => (
        <Chip key={o} active={value === o} onClick={() => onChange(o)}>{o === 'hepsi' ? 'Hepsi' : o}</Chip>
      ))}
    </div>
  );
}
...
/**
 * Stock management main page — filterable and sortable material table.
 * @param {object} props
 * @param {(kind: string, id?: string) => void} props.open - Modal / detail page open handler
 */
export function StokPage({ open }) {
  const { boruFittings, digerMalzeme, hareketler } = useStore();
