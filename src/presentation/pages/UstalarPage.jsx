...
import { TOKENS, btnPrimaryStyle, btnGhostStyle, btnDangerStyle } from '../components/ui/tokens.jsx';
...
/**
 * Workers master-detail page — left panel shows the list, right panel shows the selected worker.
 * @param {object} props
 * @param {(kind: string, id?: string) => void} props.open - Modal open handler
 */
export function UstalarPage({ open }) {
  ...
  const effectiveSelected = selected || ustalar[0]?.id || null;
  ...
  const u = ustalar.find((x) => x.id === effectiveSelected) || ustalar[0];
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '280px 1fr', gap: 20, marginTop: 16, alignItems: 'start' }}>
        <div style={{ background: TOKENS.paper, border: `1px solid ${TOKENS.line}`, borderRadius: 12, overflow: 'hidden' }}>
          {ustalar.map((usta, i) => {
            ...
            return (
              <button key={usta.id} onClick={() => setSelected(usta.id)} style={{ appearance: 'none', width: '100%', textAlign: 'left', border: 'none', borderBottom: i === ustalar.length - 1 ? 'none' : `1px solid ${TOKENS.lineSoft}`, background: effectiveSelected === usta.id ? TOKENS.bg : 'transparent', padding: '12px 14px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 10, borderLeft: effectiveSelected === usta.id ? `3px solid ${TOKENS.ink}` : '3px solid transparent' }}>
